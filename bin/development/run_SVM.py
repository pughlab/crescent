...
parser.add_argument(
    '-i', 'AlignedMatrixPath',
    type=str,
    help="The path to the mtx.gz files generated by the run_TransferData \n"
         "function."
)
...
parser.add_argument(
    '-l','TrainLabelPath',
    type=str,
    help="The path to the tsv file that contains the cell type label of the training (reference) dataset.\n"
         "The file doesn't have any header, and each row has a label that represents \n"
         "the cell type of the corresponding cell in the reference dataset."
)
...
#Read the Ref cell type labels.
y_train = pd.read_csv(args.TrainLabelPath + '/Labels.tsv', header=None)

#Change the sparse matrix to dense format and transpose the matrix
X_train = np.asarray(X_train.todense()).T

#Create and train the model with the Reference data
...
#Read the file that contains the names of the query samples
Query_list = pd.read_csv(args.AlignedMatrixPath + '/Query_names.tsv', header=None)
